...
import online.wenmeng.exception.SecretErrorMaxException;
...
public class crossDomain implements Filter {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
        String ipAddr = MyUtils.getIpAddr((HttpServletRequest) req);
        if (Config.secretErrorMap.containsKey(ipAddr)){
            if (Config.secretErrorMap.get(ipAddr)>Config.secretErrorTimes){
                throw new RuntimeException(new SecretErrorMaxException());
            }
        }
